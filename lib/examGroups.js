...
function validateExamSettings(body, current = {}, merged = {}) {
  ...
  if (has('failedRoleId')) update.failedRoleId = snowflakeField(body.failedRoleId, 'ID do cargo de reprovado', errors);
  ...
  if (autoApproval) {
    ...
    const failed = has('failedRoleId') ? update.failedRoleId : current.failedRoleId;
    ...
    if (!failed && !errors.some((e) => e.includes('reprovado'))) errors.push('ID do cargo de reprovado: obrigatório com aprovação automática.');
    if (approved && failed && approved === failed) errors.push('Os cargos de aprovado e reprovado precisam ser diferentes.');
...
function computeOutcome(exam, score, maxScore) {
  const base = {
    decidedAt: new Date(),
    score,
    maxScore,
  ...
  if (!exam || !exam.autoApproval || exam.passingScore == null) return base;
  ...
  return {
    ...
    failedRoleId: exam.failedRoleId || null,
    resultStatus: passed ? RESULT.APPROVED : RESULT.FAILED,
    resultRoleId: (passed ? exam.approvedRoleId : exam.failedRoleId) || null,
  };
}
...
module.exports = {
  GROUPS, TCEL_SLUG, SLUG_RE, RESULT, slugify, uniqueSlug, maxPossibleScore, parseBool,
  validateExamSettings, computeOutcome, findExamByRef, migrateExamGroups,
};
