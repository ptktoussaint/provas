...
function validateTemplate(template) {
  ...
  if (/["\\{}]/.test(fixed)) return 'O modelo do apelido só pode ter { } em {nome} e {idRP}, e não pode ter aspas (") nem barra invertida (\\).';
...
function validateRpInput({ nome, idRP, template }) {
  ...
  // O apelido vai dentro de um JSON montado no BotGhost: aspas e barra
  // invertida quebrariam o pedido, e { } viraria variável do BotGhost.
  if (/["\\{}]/.test(cleanNome)) errors.push('O NOME RP não pode conter aspas ("), barra invertida (\\) nem chaves ({ }).');
