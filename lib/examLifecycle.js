...
async function startOrResumeAttempt(roomId) {
  ...
  if (liveRoom) {
    for (const info of liveRoom.proctors.values()) {
      ...
      const tokenEntry = info.proctorTokenId && room.proctorTokens.id(info.proctorTokenId);
      if (tokenEntry && tokenEntry.label && !proctorNames.includes(tokenEntry.label)) {
        proctorNames.push(tokenEntry.label);
      }
    }
  }
